...
function titleFromFile(path){return path.split('/').pop().replace(/\.md$/,'').replace(/^AOS-[A-Z0-9-]+-?/,'').replace(/[-_]+/g,' ').replace(/\b\w/g,c=>c.toUpperCase())}
async function buildDocsFromTree(){const r=await fetch(`https://api.github.com/repos/${REPO}/git/trees/main?recursive=1`,{cache:'no-store'});if(!r.ok)throw Error('repository tree '+r.status);const tree=(await r.json()).tree||[];DOCS=tree.filter(x=>x.type==='blob'&&x.path.startsWith('engineering-bible/docs/')&&x.path.endsWith('.md')).map(x=>{const file=x.path.slice('engineering-bible/docs/'.length),base=file.split('/').pop().replace(/\.md$/,''),m=base.match(/^(AOS-[A-Z0-9-]+)/);return{id:m?m[1]:base.toUpperCase(),title:titleFromFile(file),summary:'',category:file.includes('/')?file.split('/')[0]:'General',status:'published',file}}).sort((a,b)=>a.id.localeCompare(b.id,undefined,{numeric:true}))}
async function loadDocs(){const r=await fetch(raw('data/docs-index.json'),{cache:'no-store'});if(r.ok){DOCS=(await r.json()).documents||[];return}await buildDocsFromTree()}
