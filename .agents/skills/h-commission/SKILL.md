...
This skill is explicit-only. Use it only when the user invokes $h-commission; treat the text after the skill name as the request context.
...
- `workspace_patch_auto_on_pass` — opt-in. On terminal+pass, the harness drain
  ...
  In `--drain` mode the agent must not also run `haft harness apply` for an
  auto-apply commission. Intervene manually only after an explicit
  `auto_apply_failed` event or an operator request.
