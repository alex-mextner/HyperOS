...
This skill is explicit-only. Use it only when the user invokes $h-onboard; treat the text after the skill name as the request context.
...
5. Draft the YAML `spec-section` block in the carrier file named by
   ...
   - `target_refs` — required for every spec section; for `target.boundary`
     sections specifically, enumerate at least four stakeholder perspectives
     (CHR-10 corners). For non-boundary sections, cite the concrete carrier(s)
     that ground the claim.
