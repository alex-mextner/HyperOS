...
The portable contracts in [the system architecture](../architecture/ARCH-001-portable-system-architecture.md#portable-boundaries) <!-- xref:AOS-ARCH-001#portable-boundaries --> are the common product. Target-specific value is measured by how much of those contracts it implements natively, not by how closely it resembles an Android phone.
...
The scoring rubric is [defined in the hardware scorecard](HW-010-hardware-scorecard.md#scoring-method) <!-- xref:AOS-HW-010#scoring-method -->.
...
6. **Pixel 9 Pro** as the primary premium camera/hardware research target under [the minimal-legacy policy](../architecture/ARCH-011-compatibility-and-pixel-legacy-policy.md#pixel-legacy-boundary) <!-- xref:AOS-ARCH-011#pixel-legacy-boundary -->.
