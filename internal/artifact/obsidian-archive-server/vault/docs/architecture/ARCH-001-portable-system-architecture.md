...
The kernel contract is owned by [AOS-ARCH-002](ARCH-002-microkernel.md#kernel-scope) <!-- xref:AOS-ARCH-002#kernel-scope -->; hardware adaptation is owned by [AOS-ARCH-006](ARCH-006-drivers-hal-and-board-packages.md#driver-model) <!-- xref:AOS-ARCH-006#driver-model -->.
...
The IDL rules are in [the IDL specification](ARCH-005-idl-api-and-versioning.md#idl-rules) <!-- xref:AOS-ARCH-005#idl-rules -->. A backend that exposes only a vendor-specific primitive may exist as an internal driver interface, but the public service contract must remain device-class oriented.
...
The same architecture must support a future contract-manufactured device without redesigning the product plane. Required early provisions are board manifests, manufacturing-test services, secure provisioning APIs, device identity and certificate lifecycle, factory diagnostics, update partitions, calibration storage, and regulatory evidence identifiers. These are specified in [the ODM-readiness document](../hardware/HW-008-custom-device-odm-readiness.md#architecture-readiness) <!-- xref:AOS-HW-008#architecture-readiness -->.
