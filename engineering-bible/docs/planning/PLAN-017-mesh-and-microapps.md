...
| AOS-MESH-006 | Prototype SX1262 accessory provider and low-power handoff | MESH-003, HW-018 | current draw, wake, recovery and provenance dossier |
...
| AOS-MESH-008 | Implement emergency profile and abuse controls | MESH-002, MESH-007, LEGAL-015 | controlled drills and wording review |
...
| AOS-MESH-010 | Decide native constrained-link protocol and integrated-device gate | MESH-009, RES-017 | decision record with retain/replace evidence |
...
| AOS-MICROAPP-005 | Implement authority diff, preview and confirmation UI | MICROAPP-001, PROD-010 | adversarial capability-escalation tests |
...
| AOS-MICROAPP-008 | Implement sharing, signing, versioning and rollback | MICROAPP-004, SEC-060 | provenance, migration and revocation tests |
...
The public portal must expose dedicated `/mesh` and `/micro-apps` pages, interactive diagrams, linked specifications, use cases and current work items. `/engineering` must index these documents inside the portal rather than routing category navigation to GitHub.
