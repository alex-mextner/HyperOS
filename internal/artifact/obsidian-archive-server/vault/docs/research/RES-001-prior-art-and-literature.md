...
Prior art informs requirements and experiments; it does not become an undocumented dependency. Every entry states what to learn, what not to assume, source IDs from [the source register](source-register.md#source-register) <!-- xref:AOS-RES-002#source-register -->, and the Agent OS contract it affects. Code reuse additionally follows [the dependency policy](../legal/LEGAL-003-open-source-ip-and-provenance.md#dependency-policy) <!-- xref:AOS-LEGAL-003#dependency-policy --> and the clean-room protocol.
