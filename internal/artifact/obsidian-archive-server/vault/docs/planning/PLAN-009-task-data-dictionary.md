...
Use document references as `AOS-DOC-ID#explicit-anchor`. Documents contain explicit HTML anchors and machine comments according to [META-001](../_meta/META-001-linking-and-publishing.md#xref-syntax) <!-- xref:AOS-META-001#xref-syntax -->. Task references use canonical IDs; the generated `TASKS.md` gives every task an anchor equal to its lowercase ID.
