...
This skill is explicit-only. Use it only when the user invokes $h-compare; treat the text after the skill name as the request context.
...
The agent computes `non_dominated_set` before calling the tool. Use the
characterized dimensions and their polarity: variant A dominates variant B only
when A is at least as good on every comparable dimension and strictly better on
at least one. Keep variants on the Pareto front when scores are missing,
incomparable, or parity warnings prevent a justified dominance claim; record the
gap in `pareto_tradeoffs` or `incomparable`.
